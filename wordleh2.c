    ...
    */
    ...
    for (int i=0; i<NB_REPONSES; i++){
        ...
        for (int j=0; j<5; j++){
            ...
            score = score + mem->frequence[indice(dico[i][j])];
...
float* calcul_frequence(char** dico){
    //cette fonction renvoie un tableau de 26 flottants qui correspondent a la frequence de chaque lettre dans le dico
    ...
    for (int i=0; i<NB_REPONSES; i++){
        for (int j=0; j<L_MOT; j++){
            freq[indice(dico[i][j])] ++;
        }
    }
    for (int i=0; i<26; i++){
        freq[i] = freq[i] / (NB_REPONSES*L_MOT);
    }
    //print_freq(freq);
    return freq;
}
...
void partie(char** dico){
    ...
    mem->frequence = malloc(sizeof(float)*26);
    mem->frequence = calcul_frequence(dico);
